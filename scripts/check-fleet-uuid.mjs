...
if (!url || !serviceRoleKey || !fleetId) {
  throw new Error(
    "SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY et FLEET_ID sont requis."
  );
}
...
else console.log("Trouvee:", JSON.stringify(flotte, null, 2));
...
const { data: health, error: hErr } = await sb.rpc(
