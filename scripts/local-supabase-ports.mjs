...
async function getFreePort(exclude = new Set()) {
  for (let attempt = 0; attempt < 200; attempt += 1) {
    const port =
      MIN_TEST_PORT +
      Math.floor(Math.random() * (MAX_TEST_PORT - MIN_TEST_PORT + 1));
...
function getLocalSupabaseProjectId() {
  ...
  return `sfa-${runPart}-${runnerPart}`.slice(0, 40).replace(/-+$/g, "");
}

export async function setLocalSupabaseTestPorts({
  ...
  updated = setTomlIntegerValueInSection(
    updated,
    "db",
    "shadow_port",
    ports.shadow
  ...
  if (disableStorage) {
    updated = setTomlBooleanValueInSection(
      updated,
      "storage",
      "enabled",
      false
...
export function restoreLocalSupabaseConfig({
  configFile = "supabase/config.toml",
  backupPath,
} = {}) {
