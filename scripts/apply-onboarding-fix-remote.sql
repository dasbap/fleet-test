-- À exécuter dans Supabase Dashboard → SQL Editor (projet lié à .env.local)
-- Corrige onboarding_progress : colonne steps_data + RLS + RPC sauvegarder/finaliser
-- Idempotent : safe à relancer

-- 1) Renommer data → steps_data si besoin
DO $$
BEGIN
...
-- 2) RLS + RPC (identique à supabase/migrations/20260521140000_onboarding_rls_and_rpc.sql)
...
CREATE OR REPLACE FUNCTION public.sauvegarder_progression_onboarding(
  p_org_id uuid, p_step integer, p_completed boolean, p_steps_data jsonb
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_step IS NULL OR p_step < 1 OR p_step > 4 THEN RAISE EXCEPTION 'etape_invalide'; END IF;
  ...
  VALUES (p_org_id, v_uid, p_step, COALESCE(p_completed, false), COALESCE(p_steps_data, '{}'::jsonb), now())
  ON CONFLICT (org_id) DO UPDATE SET
    step = EXCLUDED.step, completed = EXCLUDED.completed, steps_data = EXCLUDED.steps_data,
    updated_at = now(),
    user_id = CASE WHEN onboarding_progress.user_id = v_uid THEN onboarding_progress.user_id ELSE v_uid END
...
BEGIN
  ...
  IF NOT public.user_can_manage_org_onboarding(p_org_id) THEN
    RAISE EXCEPTION 'Permission refusée pour cette organisation.';
  END IF;
  UPDATE public.onboarding_progress SET completed = true, updated_at = now() WHERE org_id = p_org_id;
  IF NOT FOUND THEN
