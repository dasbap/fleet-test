...
if (target === "linked") {
  console.log(
    "INFO: baseline + delta validation only runs locally because it mutates the local Docker stack."
  );
  process.exit(0);
}
...
const searchFleetMigrationFile = searchFleetMigrationCandidates.find((file) =>
  existsSync(file)
);
...
let supabaseDbContainer = "supabase_db_smart-fleet-africa";

rmSync(tempRoot, { recursive: true, force: true });
rmSync(legacyRoot, { recursive: true, force: true });
mkdirSync(tempRoot, { recursive: true });
writeFileSync(
  join(tempRoot, "00000000000000_baseline_schema.sql"),
  readFileSync(baselineFile)
);
...
deltas.forEach((delta, index) => {
  ...
  const targetName = `${String(index + 1).padStart(14, "0")}_delta_${basename(
    delta
  )}`;
...
try {
  const portConfig = await setLocalSupabaseTestPorts({
    configFile,
    disableStorage: true,
  });
  supabaseDbContainer = `supabase_db_${portConfig.projectId}`;
  ...
  const searchFleetExists = dbScalar(
    "select to_regprocedure('public.search_fleet(text,integer,uuid)') is not null;"
  );
  ...
  const count = dbScalar(
    "select count(*) from public.search_fleet('test'::text, 5::int, null::uuid);"
  );
  if (!/^\d+$/.test(count)) {
  ...
  if (migrationSwapped) {
    withFsRetry(
      ...
      `Remove temporary ${migrationsRoot}`
    );
    if (existsSync(legacyRoot)) {
      withFsRetry(
        () => renameSync(legacyRoot, migrationsRoot),
        `Restore ${migrationsRoot}`
      );
    }
  }
  withFsRetry(
