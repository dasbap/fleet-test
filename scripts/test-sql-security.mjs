...
const projectId = 'smart-fleet-africa';
const dbContainerName = `supabase_db_${projectId}`;
...
const resetDatabase = process.argv.includes('--reset-database') || !process.argv.includes('--no-reset-database');

function executable(name) {
  return process.platform === 'win32' ? `${name}.cmd` : name;
}

function run(command, args, { input, capture = false, ignoreFailure = false } = {}) {
...
function dbScalar(sql) {
  const result = run(
    'docker',
    ['exec', dbContainerName, 'psql', '-U', 'postgres', '-d', 'postgres', '-tA', '-c', sql],
...
function dbContainerStatus(format) {
  const result = run('docker', ['inspect', `--format=${format}`, dbContainerName], {
    capture: true,
    ignoreFailure: true,
  });
  return result.status === 0 ? result.stdout.trim() : null;
...
function waitDbContainerHealthy(timeoutSec = 180) {
  ...
  throw new Error(`Container ${dbContainerName} is not healthy (status: ${status}).`);
}
...
function invokeLocalSqlFile(sqlFile) {
  run(
    'docker',
    ['exec', '-i', dbContainerName, 'psql', '-U', 'postgres', '-d', 'postgres', '-v', 'ON_ERROR_STOP=1'],
