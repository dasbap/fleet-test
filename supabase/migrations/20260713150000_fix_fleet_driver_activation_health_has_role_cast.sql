-- Fix has_role(uuid, unknown) ambiguity in fleet_driver_activation_health.
-- Prod can expose both has_role(uuid, role_type) and has_role(uuid, text[]).
...
COMMENT ON FUNCTION public.fleet_driver_activation_health(uuid) IS
  'Resume activation terrain par flotte : telephone profil et presence d au moins un creneau conducteur.';
