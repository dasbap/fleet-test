-- Planification de créneaux conducteurs (MVP : créneaux ponctuels)
...
-- Un seul créneau ouvert par affectation (garde-fou applicatif renforcé)
...
CREATE POLICY planning_creneaux_insert ON public.planning_creneaux
  ...
  WITH CHECK (
    public.has_role(fleet_id, 'organizer'::role_type)
    OR public.has_role(fleet_id, 'manager'::role_type)
  );
