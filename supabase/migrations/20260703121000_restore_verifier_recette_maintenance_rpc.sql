-- Restore maintenance closure readiness RPC for prod.
-- No admin bypass: access is checked through fleet roles only.
...
BEGIN
  ...
  IF NOT (
    has_role(v_job.fleet_id, 'organizer'::role_type)
    OR has_role(v_job.fleet_id, 'manager'::role_type)
    OR has_role(v_job.fleet_id, 'mechanic'::role_type)
  ) THEN
    peut_cloturer := false;
    message_blocage := 'Acces refuse a cette intervention.';
    RETURN NEXT;
    RETURN;
  END IF;
  ...
  IF v_after_count < 1 THEN
    peut_cloturer := false;
    message_blocage := 'Ajoutez au moins une photo apres intervention.';
    RETURN NEXT;
    RETURN;
  END IF;
...
COMMENT ON FUNCTION public.verifier_recette_maintenance(uuid) IS
  'Controle recette maintenance prod: roles flotte uniquement, statut in_progress, au moins une photo avant et une photo apres.';
