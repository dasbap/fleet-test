...
BEGIN
  ...
  -- Serialize activation/capacity changes for this fleet, but never cancel the
  -- fleet's other active subscriptions. They are independent entitlements.
...
BEGIN
  ...
  -- Idempotence is payment-scoped only. Never reuse another subscription from
  -- the fleet: every new payment intent owns its own subscription entitlement.
  SELECT a.id
