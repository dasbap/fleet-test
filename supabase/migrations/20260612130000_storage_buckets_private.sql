...
GRANT EXECUTE
ON FUNCTION public.is_app_super_admin()
...
BEGIN
  IF to_regclass('storage.buckets') IS NULL
    ...
    RAISE NOTICE
      'Storage schema absent - private buckets and policies setup skipped.';
    RETURN;
  END IF;
  ...
  DROP POLICY IF EXISTS "Avatars are publicly accessible"
  ON storage.objects;
  ...
  DROP POLICY IF EXISTS "incident_evidence_select_public"
  ON storage.objects;

  DROP POLICY IF EXISTS "incident_evidence_insert_authenticated"
  ON storage.objects;

  DROP POLICY IF EXISTS "incident_evidence_select_fleet"
  ON storage.objects;

  DROP POLICY IF EXISTS "incident_evidence_insert_fleet"
  ON storage.objects;

  DROP POLICY IF EXISTS "incident_evidence_delete_fleet"
  ON storage.objects;
  ...
  DROP POLICY IF EXISTS "maintenance_evidence_select_fleet"
  ON storage.objects;

  DROP POLICY IF EXISTS "maintenance_evidence_insert_fleet"
  ON storage.objects;

  DROP POLICY IF EXISTS "maintenance_evidence_delete_fleet"
  ON storage.objects;
  ...
  USING (
    ...
    AND EXISTS (
      SELECT 1
      FROM public.travaux_maintenance AS tm
      JOIN public.flotte_adhesions AS fa
        ON fa.fleet_id = tm.fleet_id
       AND fa.user_id = auth.uid()
       AND fa.is_active = true
      WHERE name LIKE
        'maintenance/' || tm.id::text || '/%'
    )
  );
  ...
  DROP POLICY IF EXISTS "tutorials_select_authenticated"
  ON storage.objects;

  DROP POLICY IF EXISTS "tutorials_insert_service"
  ON storage.objects;
