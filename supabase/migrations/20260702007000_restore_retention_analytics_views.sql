-- Restore the retention analytics runtime views expected by the dashboard.
-- Some baseline/prod states only have the later security-advisor stubs, or no
-- views at all after DROP ... CASCADE. Keep this migration idempotent and align
-- the final schema with src/types/retention-analytics.ts.
...
CREATE VIEW public.v_retention_cohorts AS
WITH membres_org AS (
  SELECT
    f.org_id,
    fa.user_id,
...
clos_agg AS (
  ...
  FROM cohorte ch
  INNER JOIN public.affectations_vehicules av ON av.driver_user_id = ch.user_id
  INNER JOIN public.flottes fl ON fl.id = av.fleet_id AND fl.org_id = ch.org_id
...
COMMENT ON VIEW public.v_retention_kpis IS
  'Retention KPIs by organization for the retention dashboard.';
COMMENT ON VIEW public.v_retention_cohorts IS
  'Weekly retention cohorts by organization for the retention dashboard.';
COMMENT ON VIEW public.v_daily_active_users IS
  'Daily active users over the last 30 UTC days by organization.';
COMMENT ON VIEW public.v_activation_funnel IS
  'Activation funnel by organization and role for the retention dashboard.';
