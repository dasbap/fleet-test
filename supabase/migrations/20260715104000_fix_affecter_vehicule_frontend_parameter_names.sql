-- Restore the RPC parameter names used by the frontend Supabase client.
-- PostgREST resolves RPC calls by named parameters, so the same uuid/uuid/uuid/timestamptz
-- signature with legacy French parameter names is not callable from AssignmentRepository.
...
COMMENT ON FUNCTION public.affecter_vehicule(uuid, uuid, uuid, timestamptz) IS
  'Affecte un vehicule avec les noms de parametres attendus par le client Supabase frontend.';
