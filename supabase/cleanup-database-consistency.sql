...
FROM (
  ...
  FROM jetons_qr jq
  LEFT JOIN vehicules v ON v.id = jq.vehicle_id
  WHERE v.id IS NULL
) t
;
...
BEGIN
  ...
  -- 10. jetons_qr sans vehicule (et optionnellement jetons expirés)
  IF p_dry_run THEN
    SELECT count(*) INTO v_count FROM jetons_qr jq
    WHERE NOT EXISTS (SELECT 1 FROM vehicules v WHERE v.id = jq.vehicle_id)
       OR jq.expires_at < now();
  ELSE
    DELETE FROM jetons_qr
    WHERE NOT EXISTS (SELECT 1 FROM vehicules v WHERE v.id = jetons_qr.vehicle_id)
       OR jetons_qr.expires_at < now();
    GET DIAGNOSTICS v_count = ROW_COUNT;
  END IF;
