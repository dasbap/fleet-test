...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  if (!SUPABASE_URL || !SERVICE_ROLE_KEY) return json(req, { ok: false, error: "server_not_configured" }, 500);

  const authHeader = req.headers.get("Authorization") ?? "";
  const token = authHeader.startsWith("Bearer ") ? authHeader.slice(7).trim() : "";
  if (!token) return json(req, { ok: false, error: "missing_authorization" }, 401);
  ...
  if (fullName.length < 2 || fullName.length > 120) return json(req, { ok: false, error: "invalid_full_name" }, 400);
  if (phone && phone.length > 64) return json(req, { ok: false, error: "invalid_phone" }, 400);
  if (!role || !VALID_ROLES.includes(role)) return json(req, { ok: false, error: "invalid_role" }, 400);

  const admin = createClient(SUPABASE_URL, SERVICE_ROLE_KEY, { auth: { persistSession: false } });
  const { data: callerData, error: callerErr } = await admin.auth.getUser(token);
  const caller = callerData?.user;
  if (callerErr || !caller) return json(req, { ok: false, error: "unauthorized" }, 401);
  ...
  if (membershipErr) return json(req, { ok: false, error: "membership_check_failed" }, 500);
  if (!callerMembership) return json(req, { ok: false, error: "forbidden_fleet_access_required" }, 403);

  const callerRole = callerMembership.role as RoleType;
  if (!canCreateRole(callerRole, role)) return json(req, { ok: false, error: "forbidden_role_assignment" }, 403);
  ...
  if (role === "organizer") {
    const { count: activeOrganizerCount, error: organizerCountErr } = await admin
      ...
      .eq("role", "organizer")
      .eq("is_active", true);
    if (organizerCountErr) return json(req, { ok: false, error: "organizer_count_check_failed" }, 500);
  ...
  const cleanupUser = async () => {
    ...
    if (error) console.error("[create-fleet-member-account] cleanup failed:", error.message);
  };
  ...
  if (existingAuthUserAttached) {
    const { data: existingMembership, error: existingMembershipError } = await admin
      ...
      .eq("user_id", userId)
      .maybeSingle();
    if (existingMembershipError) return json(req, { ok: false, error: "target_membership_check_failed" }, 500);
    if (existingMembership?.is_active) return json(req, { ok: false, error: "already_fleet_member" }, 409);
    if (existingMembership && existingMembership.role !== role && callerRole !== "organizer") {
