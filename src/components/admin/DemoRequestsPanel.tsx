...
interface DemoRequestsPanelProps {
  onCreateAccess: (payload: CreateDemoPayload) => Promise<{ ok: boolean; user_id?: string; magic_url?: string; error?: string }>;
      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-2"><Switch id="demo-auto-mode" checked={autoSettings.enabled} onCheckedChange={(enabled) => saveAutoSettings({ enabled, decision: autoSettings.decision })} /><Label htmlFor="demo-auto-mode">Decision automatique apres 48h</Label></div>
          </div>
          <div className="flex flex-col gap-2 sm:flex-row">
            <Select value={autoSettings.decision} onValueChange={(decision: DemoRequestAutoDecision) => saveAutoSettings({ enabled: autoSettings.enabled, decision })}><SelectTrigger className="w-[180px]"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="refuse">Refuser auto</SelectItem><SelectItem value="accept">Accepter auto</SelectItem></SelectContent></Select>
            <Button type="button" variant="outline" onClick={() => void refetch()}><RefreshCw className="mr-2 h-4 w-4" aria-hidden />Actualiser</Button>
            <Button type="button" variant="ghost" onClick={() => setIncludeProcessed((value) => !value)}>{includeProcessed ? "Masquer traitees" : "Voir traitees"}</Button>
          </div>
        </CardContent>
      </Card>
        ...
        <div className="grid gap-3">
          {requests.map((request) => (
            <Card key={request.id}><CardContent className="space-y-3 py-4">
              <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between"><div><p className="font-medium">{request.full_name}</p><p className="text-sm text-muted-foreground">{request.email}</p><p className="text-xs text-muted-foreground">{buildCompanyLabel(request)}</p><dl className="mt-2 grid gap-1 text-xs text-muted-foreground sm:grid-cols-2">{buildRequestDetails(request).map((detail) => <div key={detail.label}><dt className="font-medium text-foreground">{detail.label}</dt><dd>{detail.value}</dd></div>)}</dl></div><span className="rounded-md border px-2 py-1 text-xs uppercase">{request.status}</span></div>
              {request.message ? <div className="rounded-md border bg-muted/30 p-3 text-sm text-muted-foreground">{request.message}</div> : null}
              {request.status === "pending" ? <><Input value={reasonById[request.id] ?? ""} onChange={(event) => setReasonById((current) => ({ ...current, [request.id]: event.target.value }))} placeholder="Note interne ou raison de decision" /><div className="flex flex-col gap-2 sm:flex-row"><Button type="button" onClick={() => void acceptRequest(request)} disabled={busyId === request.id || finalizeRequest.isPending}>{busyId === request.id ? <Loader2 className="mr-2 h-4 w-4 animate-spin" aria-hidden /> : <Check className="mr-2 h-4 w-4" aria-hidden />}Accepter et creer le compte</Button><Button type="button" variant="outline" onClick={() => void refuseRequest(request)} disabled={finalizeRequest.isPending}><X className="mr-2 h-4 w-4" aria-hidden />Refuser</Button></div></> : null}
            </CardContent></Card>
          ))}
        </div>
      )}
