...
export function ContactDemoForm({ className }: ContactDemoFormProps) {
  ...
  if (sent) {
    return <div className={className}><div className="py-8 text-center"><CheckCircle2 className="mx-auto mb-4 h-12 w-12 text-primary" /><h3 className="mb-2 text-xl font-heading font-bold">Demande envoyée !</h3><p className="text-sm text-muted-foreground">Votre demande est transmise aux admins. Vous recevrez un email quand votre accès sera accepté ou refusé.</p></div></div>;
  }
  ...
  return (
    <form onSubmit={(event) => void handleSubmit(event)} className={className}>
      ...
      <div className="space-y-4">
        ...
        <div><Label htmlFor="demo-email">Adresse mail *</Label><Input id="demo-email" required type="email" autoComplete="email" value={form.email} onChange={(event) => setForm({ ...form, email: event.target.value })} placeholder="vous@entreprise.com" className="mt-1" /></div>
        ...
        <Button type="submit" className="w-full gap-2" disabled={submitDemoRequest.isPending}>{submitDemoRequest.isPending ? "Envoi..." : "Demander ma démo"}<ArrowRight className="h-4 w-4" /></Button>
      </div>
    </form>
  );
}
