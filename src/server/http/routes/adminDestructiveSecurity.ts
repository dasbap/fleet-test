...
async function deleteFleet(c: Context) {
  ...
  if (fleetError) {
    return c.json({ ok: false, error: "fleet_lookup_failed", detail: fleetError.message }, 502);
  }
  ...
  if (deleteError) {
    return c.json({ ok: false, error: "delete_fleet_failed", detail: deleteError.message }, 502);
  }

  return c.json({ ok: true, fleet_id: fleetId, fleet_name: fleet.name ?? null }, 200);
}
...
async function deleteUser(c: Context) {
  ...
  if (targetError || !targetData.user) {
    return c.json({ ok: false, error: "user_not_found", detail: targetError?.message }, 404);
  }
  ...
  if (profileError) {
    return c.json({ ok: false, error: "admin_profile_lookup_failed", detail: profileError.message }, 502);
  }
  ...
  if (organizerError) {
    return c.json({ ok: false, error: "organizer_membership_lookup_failed", detail: organizerError.message }, 502);
  }
