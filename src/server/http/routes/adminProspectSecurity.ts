...
const createProspectSchema = z.object({
  ...
  company_identifier: z.string().trim().min(1).max(200),
...
async function handleSecureLocalProspect(c: Context) {
  ...
  if (parsed.data.send_email) {
    ...
    if (notificationError) console.error("[admin-prospect-security] welcome notification failed:", notificationError.message);
  }
