...
async function createProspectLocally(
  ...
  const { data: markerData, error: markerError } = await admin.auth.admin.updateUserById(userId, {
    app_metadata: {
  ...
  const { data: verifiedUser, error: verificationError } = await admin.auth.admin.getUserById(userId);
  if (verificationError || verifiedUser.user.app_metadata?.must_set_password !== true) {
    await admin.auth.admin.deleteUser(userId);
    return c.json({ ok: false, error: "must_set_password_verification_failed" }, 500);
  }
  ...
  if (body.send_email) {
    ...
    if (notificationError) {
      console.error("[admin-demo] notification queue failed:", notificationError.message);
    }
  }
