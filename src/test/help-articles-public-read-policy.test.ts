...
describe("help articles public read policy", () => {
  ...
  it("lets platform admins write FAQ help articles without fleet membership", () => {
    const sql = readFileSync(
      join(process.cwd(), "supabase/migrations/20260803125000_fix_help_articles_platform_admin_write.sql"),
      "utf8",
    ).toLowerCase();

    expect(sql).toMatch(/create\s+policy\s+help_articles_admin_insert[\s\S]*public\.is_platform_admin\(\)/);
    expect(sql).toMatch(/create\s+policy\s+help_articles_admin_update[\s\S]*public\.is_platform_admin\(\)/);
    expect(sql).toMatch(/with\s+check\s*\([\s\S]*public\.is_platform_admin\(\)/);
    expect(sql).toMatch(/create\s+policy\s+help_articles_admin_delete[\s\S]*public\.is_platform_admin\(\)/);
    expect(sql).toContain("grant insert, update, delete on public.help_articles to authenticated");
  });

  it("adds an admin FAQ upsert RPC to avoid direct help_articles PATCH RLS failures", () => {
    const sql = readFileSync(
      join(process.cwd(), "supabase/migrations/20260803130000_admin_upsert_faq_article_rpc.sql"),
      "utf8",
    ).toLowerCase();

    expect(sql).toContain("create or replace function public.admin_upsert_faq_article");
    ...
    expect(sql).toContain("grant execute on function public.admin_upsert_faq_article");
  });

  it("lets admins select hidden FAQ help articles in the admin module", () => {
    const sql = readFileSync(
      join(process.cwd(), "supabase/migrations/20260803131000_help_articles_admin_select_hidden.sql"),
      "utf8",
    ).toLowerCase();
    ...
    expect(sql).toContain("grant select on public.help_articles to authenticated");
  });
});
