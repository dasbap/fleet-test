...
describe("BillingService", () => {
  it("mappe correctement le snapshot billing", async () => {
    ...
    vi.spyOn(repository, "findActiveSubscriptionByFleetId").mockResolvedValue({
      id: "sub-1",
      status: "active",
    ...
    vi.spyOn(repository, "findLatestPaymentsByOrgId").mockResolvedValue([
      {
        id: "pay-1",
        provider: "momo",
        amount: 12000,
  ...
  it("retourne un snapshot vide si RLS refuse l'accès billing", async () => {
    const repository = new BillingRepository();
    vi.spyOn(repository, "findActiveSubscriptionByFleetId").mockRejectedValue(
