...
type PasswordChangeError = Error & { code?: string; status?: number; weakPassword?: { reasons?: string[]; message?: string } };
type PasswordFunctionResult = { ok?: boolean; error?: string; details?: string };
...
function getPasswordErrorMessage(error: PasswordChangeError): string {
  switch (error.code) {
    ...
    case "weak_password": {
      ...
      if (reasons?.length) return `Le mot de passe est trop faible : ${reasons.join(", ")}.`;
...
export default function SetPasswordPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!user.email) {
      setError("L'adresse e-mail du compte est indisponible.");
      return;
    }
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-background p-4">
      <form onSubmit={handleSubmit} className="w-full max-w-md space-y-5 rounded-xl border bg-card p-6 shadow-sm">
        <div>
          <h1 className="text-2xl font-semibold">Créez votre mot de passe</h1>
          <p className="mt-2 text-sm text-muted-foreground">Choisissez un mot de passe différent du mot de passe temporaire.</p>
        </div>
        <div className="space-y-2">
          <Label htmlFor="password">Nouveau mot de passe</Label>
          <Input id="password" type="password" autoComplete="new-password" placeholder="Nouveau mot de passe" value={password} onChange={(event) => setPassword(event.target.value)} disabled={isSubmitting} minLength={MIN_PASSWORD_LENGTH} required />
        </div>
        <div className="space-y-2">
          <Label htmlFor="confirmation">Confirmer le mot de passe</Label>
          <Input id="confirmation" type="password" autoComplete="new-password" placeholder="Confirmer le mot de passe" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} disabled={isSubmitting} minLength={MIN_PASSWORD_LENGTH} required />
        </div>
        {error ? <p className="text-sm text-destructive" role="alert">{error}</p> : null}
        <Button type="submit" className="w-full" disabled={isSubmitting}>{isSubmitting ? "Enregistrement..." : "Créer mon mot de passe"}</Button>
      </form>
    </main>
  );
}
