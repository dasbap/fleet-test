...
// ─── Formats attendus ──────────────────────────────────────────────────────────
// Nouveau format CSPRNG : SAMBA-INV-4F2A9C0187B6D3E1
// Format historique encore accepté jusqu'à rotation : SAMBA-INV-ABC-0042

const SECURE_ACCESS_CODE_REGEX = /^SAMBA-(?:INV|PRO|COM|DEV)-[A-F0-9]{16}$/;
const LEGACY_ACCESS_CODE_REGEX = /^[A-Z]+-[A-Z0-9]+-[A-Z0-9]+-\d{4}$/;
...
export function validateCodeFormat(raw: string): string | null {
  ...
  if (code.length < 8) {
    return "Le code est trop court. Exemple : SAMBA-INV-4F2A9C0187B6D3E1";
  }
  ...
  if (!SECURE_ACCESS_CODE_REGEX.test(code) && !LEGACY_ACCESS_CODE_REGEX.test(code)) {
    return "Format de code invalide. Vérifiez le code fourni par E-Samba.";
  }
