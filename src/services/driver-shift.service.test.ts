...
describe("DriverShiftService.reviewClosure", () => {
  const validatedClosure = {
    ...
    validated_at: "2026-05-31T18:00:00.000Z",
    created_at: "2026-05-31T17:00:00.000Z",
  };

  it("valide une clôture pending", async () => {
    const updateClosure = vi.fn().mockResolvedValue(validatedClosure);
    const service = makeService({ updateClosure });

    const result = await service.reviewClosure("closure-1", "validated", "manager-1");

    expect(updateClosure).toHaveBeenCalledWith("closure-1", {
      status: "validated",
      validated_by: "manager-1",
      validated_at: expect.any(String),
    });
    expect(result).toBe(validatedClosure);
  });

  it("rejette une clôture pending", async () => {
    const updateClosure = vi.fn().mockResolvedValue({
      ...validatedClosure,
      status: "rejected",
    });
    const service = makeService({ updateClosure });

    await service.reviewClosure("closure-1", "rejected", "manager-1");
  ...
  it("exige closureId et validatedBy", async () => {
    const service = makeService({ updateClosure: vi.fn() });

    await expect(service.reviewClosure("", "validated", "manager-1")).rejects.toThrow(
      /clôture/i,
    );
    await expect(service.reviewClosure("closure-1", "validated", "")).rejects.toThrow(
      /validateur/i,
    );
  });
  ...
  it("rejette un statut invalide", async () => {
    ...
    await expect(
      service.reviewClosure("closure-1", "pending" as "validated", "manager-1"),
    ).rejects.toThrow(/validated ou rejected/i);
  });
});
